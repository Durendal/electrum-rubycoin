...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-rubycoin-setup.exe"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-rubycoin.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-rubycoin.exe" "" "$INSTDIR\electrum-rubycoin.exe" 0

  ;Links rubycoin: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\rubycoin" "" "URL:rubycoin Protocol"
  WriteRegStr HKCU "Software\Classes\rubycoin" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\rubycoin" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\rubycoin\shell\open\command" "" "$\"$INSTDIR\electrum-rubycoin.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\rubycoin"
