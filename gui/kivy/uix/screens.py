...
from electrum_rubycoin.util import profiler, parse_URI, format_time, InvalidPassword, NotEnoughFunds
from electrum_rubycoin import rubycoin
from electrum_rubycoin.util import timestamp_to_datetime
...
class SendScreen(CScreen):
    ...
    def set_URI(self, text):
        ...
        try:
            ...
            self.app.show_info(_("Not a Rubycoin URI"))
            return
    ...
    def do_save(self):
        ...
        from electrum_rubycoin.paymentrequest import make_unsigned_request, PaymentRequest
    ...
    def do_send(self):
        if self.screen.is_pr:
            ...
            if not address:
                self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a Rubycoin address or a payment request'))
                return
            if not rubycoin.is_address(address):
                self.app.show_error(_('Invalid Rubycoin Address') + ':\n' + address)
                return
            try:
                ...
                return
            outputs = [(rubycoin.TYPE_ADDRESS, address, amount)]
...
class ReceiveScreen(CScreen):
    ...
    def get_URI(self):
        from electrum_rubycoin.util import create_URI
    ...
    def do_share(self):
        uri = self.get_URI()
        self.app.do_share(uri, _("Share Rubycoin Request"))
